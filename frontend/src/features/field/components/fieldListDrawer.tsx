...
export default function FieldListDrawer({ farmId, onEditField, onDeleteField }: Props) {
  const [isOpen, setIsOpen] = useState(false)
  const { getFieldsByFarmId, updateField } = useFieldStore()

  const fields = farmId ? getFieldsByFarmId(farmId) : []
    ...
    <>
      ...
      >
        <div className="px-4 py-3 border-b border-[#e0e8d8] bg-[#f5f8f0] flex items-center justify-between shrink-0">
          <div>
            <p className="text-xs font-semibold text-[#2d4a1e] uppercase tracking-wide">Campos</p>
          ...
          {fields.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full gap-2 px-6 text-center">
              ...
              <p className="text-xs text-[#9aab8a]">
                No hay campos. Añade tu primer campo desde el mapa.
              </p>
            </div>
          ) : (
  ...
  return (
    <div className="px-4 py-3 hover:bg-[#fafcf8] transition-colors">
      <div className="flex items-center gap-2 mb-1">
        ...
        <span className="text-sm font-medium text-[#2d4a1e] truncate flex-1">{field.name}</span>
      </div>
      ...
      <button onClick={onToggleDisplay}
      ...
      >
        {field.displayMode === 'pin' ? (
          <><MapPin size={11} className="text-[#639922]" /><span>Mostrar como pin</span><ToggleLeft size={14} className="text-[#c0d0b0] ml-auto" /></>
        ) : (
          <><Square size={11} className="text-[#639922]" /><span>Mostrar como forma</span><ToggleRight size={14} className="text-[#639922] ml-auto" /></>
        )}
      </button>
      ...
      {!confirmDelete ? (
        ...
        <div className="flex flex-col gap-1.5">
          <p className="text-[10px] text-red-500 text-center">¿Eliminar "{field.name}"?</p>
          <div className="flex items-center gap-2">
