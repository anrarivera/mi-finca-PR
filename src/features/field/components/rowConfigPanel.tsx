...
export default function RowConfigPanel({
  ...
  const [primaryCropId, setPrimaryCropId] = useState<string>('')
  const [companionCropId, setCompanionCropId] = useState<string>('')
  const [spacingFt, setSpacingFt] = useState(6)
  const [plantCount, setPlantCount] = useState(0)

  // Recalculate preview count when settings change
  useEffect(() => {
    if (!primaryCropId) { setPlantCount(0); return }
    const plants = calculateRowPlants({
      id: 'preview',
      startX: rowDraft.startX,
      startY: rowDraft.startY,
      endX: rowDraft.endX,
      endY: rowDraft.endY,
      spacingFt,
      widthFt,
    ...
    <div className="absolute right-4 top-4 z-10 w-60 bg-white rounded-xl border border-[#e0e8d8] shadow-lg overflow-hidden">
      ...
      <div className="p-4 flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <CropSelector
            value={companionCropId}
            onChange={setCompanionCropId}
            placeholder="Sin compañera"
            allowClear
          />
          {companionCropId && (
            <p className="text-[10px] text-[#9aab8a] leading-relaxed">
              Las plantas se alternarán: principal → compañera → principal → compañera...
            </p>
          )}
        </div>
